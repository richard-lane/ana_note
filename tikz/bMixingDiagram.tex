...
\begin{tikzpicture}
    ...
    %\settowidth{\dwidth}{\pgfinterruptpicture $\overline{K^-D}^0$ \endpgfinterruptpicture}
    %\tikzmath{
    %\shift=\dwidth/-2;}
    ...
    \draw[-latex] (D) edge[out=0, in=135] node[above right]{} (f);

    \draw[-latex] (B) edge[out=-45, in=180] node[below left]{} (Dbar);
    \draw[-latex, line width=1.5mm] (Dbar) edge[out=0, in=-135] node[below right]{} (f);
\end{tikzpicture}
\end{document}
